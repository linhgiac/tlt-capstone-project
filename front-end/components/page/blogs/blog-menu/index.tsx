...
import {
    ...
    SKILLS,
    SUMMARY_OBJECTIVE,
} from '../../../../configs/constants/blog.constants';
...
const itemsHasTableOfContents: MenuItem[] = [
    getItem(
        'Table of contents',
        'tableofcontents',
        <MenuUnfoldOutlined />,
...
const BlogMenu = ({ hasTableOfContents = false }: BlogMenuProps) => {
    ...
    return (
        <Menu
            onClick={onClick}
            style={hasTableOfContents ? {} : { height: '100%' }}
            defaultOpenKeys={['fundamentals']}
            selectedKeys={[current]}
            mode="inline"
            items={hasTableOfContents ? itemsHasTableOfContents : items}
        />
    );
};
